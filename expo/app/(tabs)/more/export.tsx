...
import { Download, Users, FileText, Check, Shield } from 'lucide-react-native';
import { useColors } from '@/providers/ThemeProvider';
import { ThemeColors } from '@/constants/colors';
import { useAuth } from '@/providers/AuthProvider';
import { useParking } from '@/providers/ParkingProvider';
import { formatDateTime } from '@/utils/helpers';
import { shareCsv } from '@/utils/file-save';
...
export default function ExportScreen() {
  ...
  const { activeClients, activeCars, transactions, getClientDebtTotal } = useParking();
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.exportCard}>
        <View style={styles.exportHeader}>
          ...
          <Text style={styles.exportTitle}>Список клиентов</Text>
        </View>
        <Text style={styles.exportDesc}>
          {activeClients.length} клиентов · {activeCars.length} машин
        </Text>
        <TouchableOpacity
          style={[styles.exportBtn, clientsExported && styles.exportBtnDone]}
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  exportHeader: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 12 },
  exportTitle: { fontSize: 17, fontWeight: '700' as const, color: colors.text },
  exportDesc: { fontSize: 13, color: colors.textSecondary, marginBottom: 16 },
